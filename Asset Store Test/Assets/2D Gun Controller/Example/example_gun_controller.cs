...
/*=============================================================================
*   This is an "example" version of the gun_controller script available at the
*   root of this asset. This script is almost identical to the production
*   gun_controller script, except that this script has example connections
*   to the player GameObject's animator component as well as other connections
*   that are outline in the production script.
*
*   Because this is an "example" version, using this script directly in your
*   project may cause unforseen errors as this script is set up to work in the
*   Example scene provided with this project.
*
*   The area of this script that differs from the production script is:
*       - void FixedUpdate() at line 194
=============================================================================*/

public class example_gun_controller : MonoBehaviour {
    ...
    //Accuracy Properties
    //=================================================
    [SerializeField] private bool accuracy_enabled                                  = true;  //Enable or Disable gun accuracy.
    ...
    //Recoil properties
    //=================================================
    [SerializeField] private bool recoil_enabled            = true;     //Enable or Disable gun recoil.
    ...
    void FixedUpdate()
    {
		if (get_accuracy_enabled())
        {
			/*  ==========
                ...
                ==========
            */
            if (get_player() != null && get_player_animator() != null)
            {
                set_is_airborne(!get_player_animator().GetBool("Grounded"));
                set_is_standing(get_player_animator().GetBool("Standing"));
                set_is_crouching(get_player_animator().GetBool("Crouching"));
                set_is_walking(get_player_animator().GetBool("Walking"));
                set_is_running(get_player_animator().GetBool("Sprinting"));
            }
            else
            {
                Debug.LogError("Missing Animator Reference for player_animator. Try adding a GameObect to the 'Player' variable through the Inspector Window.");
            }
        }
    }
